...
class EDXS_Table () :
    ...
    def first_ionization(self,element,shell):
        ...
        try :
            energy = self.low_OV_table.loc[(self.low_OV_table["Z"]==element)&(self.low_OV_table["S"]==shell)].iloc[0]["E"]
            ...
            if U < 16 :
                ...
                cs = U/(U + b_)*A*((np.log(np.power(X,2))-B**2)*(1+g1/X) + g2 + g3*np.power((1-B**2),-0.25)+ g4/X)
        ...
        if K :
            ...
            for i in range(1,int(L.group(2))) :
                try :
                    ...
                    cs += self.first_ionization(element,string_shell)*xr.CosKronTransProb(element,self.xr_dict[string_CK])
                except ValueError :
                    pass
    ...
    def wernisch_abs_table(self):
        ...
        for elt in self.elements :
            ...
            try : 
                ...
                L = True
                for shell in list_L :
                    shells_dict[shell] = [xr.EdgeEnergy(elt,self.xr_dict[shell+"_SHELL"]),self.wernisch_H[shell]]
            except ValueError :
                pass
            try : 
                ...
                pass
            if M and L : 
                d = [self.wernisch_dK(elt),self.wernisch_dL(elt),self.wernisch_dM(elt),self.wernisch_dN(elt)]
                k = [-2.685,-2.669,-2.514,-2.451]
            elif L and not(M) :
                d = [self.wernisch_dK(elt),self.wernisch_dL(elt),self.wernisch_dM(elt)]
                k = [-2.685,-2.669,-2.514]
            else : 
    ...
    def wernisch_dK (self, Z) :
        return 5.955 + 3.917e-1*Z - 1.054e-2*np.power(Z,2) + 1.520e-4*np.power(Z,3) - 8.508e-7*np.power(Z,4)

    def wernisch_dL (self,Z) :
        return 3.257 + 3.936e-1*Z - 8.483e-3*np.power(Z,2) + 9.491e-5*np.power(Z,3) - 4.058e-7*np.power(Z,4)

    def wernisch_dM (self,Z) :
        return 2.382 + 2.212e-1*Z - 2.028e-3*np.power(Z,2) + 6.891e-6*np.power(Z,3)
