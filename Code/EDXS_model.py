        ...
        for elt in elements_dict.keys() :
            for shell in self.xrays[elt] :
                for i,energy in enumerate(self.xrays[elt][shell]["energies"]) :
                    width = self.width_slope * energy + self.width_intercept
                    temp += elements_dict[elt]*self.xrays[elt][shell]["ratios"][i]*u.Functions.gaussian(self.x,energy,width/2.3548)
            ...
            for shell in self.abs_db[elt] :
                ...
                if regex : 
                    if regex.group(1) == "K" :
                        ...
                        dk_tups.append((ind_dk,self.abs_db[elt]["d"][0],self.abs_db[elt]["exp"][0]))

                    if regex.group(0) == "L" + str(max_ind_L) :
                        en = self.abs_db[elt][shell][0]
                        ind_dk = np.argmin(np.abs(self.x - en))
                        dk_tups.append((ind_dk,self.abs_db[elt]["d"][1],self.abs_db[elt]["exp"][1]))

                    if regex.group(0) == "M" + str(max_ind_M) :
                        en = self.abs_db[elt][shell][0]
                        ind_dk = np.argmin(np.abs(self.x - en))
                        dk_tups.append((ind_dk,self.abs_db[elt]["d"][2],self.abs_db[elt]["exp"][2]))
