...
from espm.utils import number_to_symbol_list, get_explained_intensity_W
import numpy as np
...
class EDS_espm(Signal1D) : 
    ...
    def estimate_best_binning(self, bin_sampling = 20, inspect = False) :
        r"""
        ...
        binx = np.linspace(1, self.axes_manager[0].size//2, bin_sampling)
        biny = np.linspace(1, self.axes_manager[1].size//2, bin_sampling)

        vars_est = np.array([])
        biases_est = np.array([])

        for i in tqdm(range(bin_sampling)):
            # Bin the measurement dataset and upsample to bring it back to its original dimensionality
            B = binx[i]*biny[i]
            binned = self.rebin(scale = (binx[i], biny[i], 1))
        ...
        estimated_binning = (binx[np.argmin(mprimes_est)], biny[np.argmin(mprimes_est)],1)
        if inspect :
