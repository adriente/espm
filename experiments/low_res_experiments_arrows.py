...
for i in range(len(downsampling_factors)):
    ...
    est = SmoothNMF( n_components = 3,tol=0.0001, max_iter = 1000, G = None, lambda_L=0, mu=0, force_simplex=False, init="nndsvdar", normalize=True)
...
X = spim.X
est = SmoothNMF( n_components = 3,tol=0.0001, max_iter = 1000, G = None, lambda_L=10, mu=10, force_simplex=False, init="nndsvdar", normalize=True)
Ximg = X.reshape((-1, pixels_side,pixels_side))
Hflat =None
W = None
...
X = spim.X
est = SmoothNMF( n_components = 3,tol=0.0001, max_iter = 1000, G = None, lambda_L=1000, mu=10, force_simplex=False, init="nndsvdar", normalize=True)
Ximg = X.reshape((-1, pixels_side,pixels_side))
Hflat =None
W = None
...
for i in range(len(downsampling_factors)):
    ...
    est = SmoothNMF( n_components = 3,tol=0.0001, max_iter = 1000, G = None, lambda_L=1000, mu=10, force_simplex=False, init="nndsvdar")
...
X = spim.X
est = SmoothNMF( n_components = 3,tol=0.0001, max_iter = 1000, G = None, lambda_L=10, mu=1000, force_simplex=False, init="nndsvdar")
Ximg = X.reshape((-1, pixels_side,pixels_side))
Hflat =None
W = None
...
plt.scatter(r2_max_exp[:len(downsampling_factors)], angle_max_exp[:len(downsampling_factors)])
plt.scatter(r2_max_exp[len(downsampling_factors):2*len(downsampling_factors)], angle_max_exp[len(downsampling_factors):2*len(downsampling_factors)])
plt.scatter(r2_max_exp[2*len(downsampling_factors):3*len(downsampling_factors)], angle_max_exp[2*len(downsampling_factors):3*len(downsampling_factors)])
plt.scatter(r2_max_exp[len(downsampling_factors)*3:], angle_max_exp[len(downsampling_factors)*3:])


plt.title("Regularisation parameters change with the image scale")
...
plt.quiver([r2_max_exp[i] for i in range(len(downsampling_factors))],[angle_max_exp[i] for i in range(len(downsampling_factors))],[r2_max_exp[i+len(downsampling_factors)]-r2_max_exp[i] for i in range(len(downsampling_factors))],[angle_max_exp[i+len(downsampling_factors)]-angle_max_exp[i] for i in range(len(downsampling_factors))],  angles='xy', scale_units='xy', scale=1, headaxislength=0, **quiveropts)
plt.quiver([r2_max_exp[i] for i in range(len(downsampling_factors))],[angle_max_exp[i] for i in range(len(downsampling_factors))],[r2_max_exp[i+len(downsampling_factors)*2]-r2_max_exp[i] for i in range(len(downsampling_factors))],[angle_max_exp[i+len(downsampling_factors)*2]-angle_max_exp[i] for i in range(len(downsampling_factors))],  angles='xy', scale_units='xy', scale=1, headaxislength=0, **quiveropts)
plt.quiver([r2_max_exp[i] for i in range(len(downsampling_factors))],[angle_max_exp[i] for i in range(len(downsampling_factors))],[r2_max_exp[i+len(downsampling_factors)*3]-r2_max_exp[i] for i in range(len(downsampling_factors))],[angle_max_exp[i+len(downsampling_factors)*3]-angle_max_exp[i] for i in range(len(downsampling_factors))],  angles='xy', scale_units='xy', scale=1, headaxislength=0, **quiveropts)
plt.legend(["baseline", "($\lambda$ = 10 $\mu = 10$)", "($\lambda$ = 1000 $\mu = 10$)", "($\lambda$ = 10 $\mu = 1000$)"])
# annotations = ['baseline', 'regulatization ($\lambda$ = 10, $\mu$ = 10)', 'smoothing median filter of size (1,10,10)', 'downsampling (16->8->4->2->1)']
...
lambdas = [10**((i//n)+2) for i in range(n*m)]
mus = [10**((i%n)+2) for i in range(n*m)]
for j in range(n*m):
    ...
    for i in range(len(downsampling_factors)):
        ...
        D , Hs, Ws = est.fit_transform(Xflat)
