...
class SNMF:
    ...
    def _eval_function(self, d_matr=None):
        ...
        if (self.mu_sparse != 0.0) and not (self.sparse_stop):
            sparse_loss = self.mu_sparse * np.sum(
                np.log(self.a_matr[1:, :] + self.eps_sparse)
            )
        else:
            sparse_loss = 0.0
    ...
    def dichotomy_brstlg(self, u, v, tol):
        ...
        while np.any(np.abs(lambda_pos - lambda_neg) > tol):
            ...
            expr = (
                np.power((u / (1 + 2 * lambda_pos)), 2) - 4 * v - 16 * lambda_pos
            ) * (np.power((u / (1 + 2 * new)), 2) - 4 * v - 16 * new)
        ...
        if self.mu_sparse != 0:
            ...
            if mask is None:
                ...
                self.a_matr = (
                    ...
                    / (
                        V[:, np.newaxis]
                        + vec_sparse[:, np.newaxis] / (self.fixed_a + self.eps_sparse)
                        + self.lambda_s
                    )
                    * U
                )
                ...
                # Lagragian multiplier
                self.lambda_s = self.dichotomy_simplex(
                ...
                self.a_matr[n_mask] = (
                    self.a_matr[n_mask]
                    / (V[:, np.newaxis] + self.lambda_s)[n_mask]
                    * U[n_mask]
                )
    ...
    def _make_step_p(self):
        ...
        d_a = self.d_matr @ self.a_matr
        term1 = (
            self.g_matr.T @ (self.x_matr.clip(min=1e-150) / (d_a)) @ self.a_matr.T
        )  # The data are clipped to avoid issues during computation
        term2 = (
            self.g_matr.sum(axis=0)[:, np.newaxis]
            @ self.a_matr.sum(axis=1)[:, np.newaxis].T
        )
        # P update
    ...
    def _make_step_b(self):
        ...
        d_tilde = self.g_matr @ self.p_matr + self.calc_b(
            c0=c0_tilde, c1=c1_tilde, c2=c2_tilde
        )
        func_c = self._eval_function(d_matr=d_tilde)
        # Line search algorithm to determine gradient step size
        while func_c > (
            cur_loss
            + self.f_c_quadr(c0_tilde, c1_tilde, c2_tilde, grad_c0, grad_c1, grad_c2)
        ):
        ...
        # Line search algorithm to determine gradient step size
        while func_b1 > (
            cur_loss + self.f_b_quadr(b1_tilde, b2_tilde, grad_b1, grad_b2)
        ):
            # At each step the eta increase, which reduces the step size
    ...
    def calc_b(self, b0=None, b1=None, b2=None, c0=None, c1=None, c2=None):
        ...
        if c2 is None:
            c2 = self.c2
        return (
            self.chapman_brstlg(b0, b1, b2) * self.detector(c1, c2) * self.self_abs(c0)
        )
    ...
    def detector(self, c1=None, c2=None):
        ...
        if c2 is None:
            c2 = self.c2
        return np.exp(-c2 / np.power(self.E[:, np.newaxis], 3)) * (
            1 - np.exp(-c1 / np.power(self.E[:, np.newaxis], 3))
        )
    ...
    def self_abs(self, c0=None):
        ...
        if c0 is None:
            c0 = self.c0
        return (
            np.power(self.E[:, np.newaxis], 3)
            * (1 - np.exp(-c0 / np.power(self.E[:, np.newaxis], 3)))
            / c0
        )
    ...
    def calc_dc0(self):
        ...
        return (
            ...
            * np.power(self.E[:, np.newaxis], 3)
            / np.power(self.c0, 2)
            * (
                np.exp(-self.c0 / np.power(self.E[:, np.newaxis], 3))
                - 1
                + np.exp(-self.c0 / np.power(self.E[:, np.newaxis], 3))
                * self.c0
                / np.power(self.E[:, np.newaxis], 3)
    ...
    def calc_dc1(self):
        ...
        return (
            self.chapman_brstlg()
            * self.self_abs()
            * np.exp(-self.c2 / np.power(self.E[:, np.newaxis], 3))
            * np.exp(-self.c1 / np.power(self.E[:, np.newaxis], 3))
            / np.power(self.E[:, np.newaxis], 3)
        )
    ...
    def calc_dc2(self):
        ...
        return (
            ...
            * self.self_abs()
            * np.exp(-self.c2 / np.power(self.E[:, np.newaxis], 3))
            * (1 - np.exp(-self.c1 / np.power(self.E[:, np.newaxis], 3)))
            / np.power(self.E[:, np.newaxis], 3)
        )
    ...
    def dLdB(self):
        ...
        """
        return -(
            self.x_matr.clip(min=1e-150) / (self.d_matr @ self.a_matr)
        ) @ self.a_matr.T + np.sum(self.a_matr, axis=1)
    ...
    def f_b_quadr(self, b1_tilde, b2_tilde, grad_b1, grad_b2):
        ...
        """
        return (
            grad_b1 @ (b1_tilde - self.b1)
            + grad_b2 @ (b2_tilde - self.b2)
            + grad_b1 @ (b1_tilde - self.b1)
            + self.eta_b1 / 2 * np.linalg.norm(b1_tilde - self.b1) ** 2
            + self.eta_b2 / 2 * np.linalg.norm(b2_tilde - self.b2) ** 2
        )
    ...
    def f_c_quadr(self, c0_tilde, c1_tilde, c2_tilde, grad_c0, grad_c1, grad_c2):
        ...
        """
        c0_part = (
            grad_c0 @ (c0_tilde - self.c0)
            + self.eta_c0 / 2 * np.linalg.norm(c0_tilde - self.c0) ** 2
        )
        c1_part = (
            grad_c1 @ (c1_tilde - self.c1)
            + self.eta_c1 / 2 * np.linalg.norm(c1_tilde - self.c1) ** 2
        )
        c2_part = (
            grad_c2 @ (c2_tilde - self.c2)
            + self.eta_c2 / 2 * np.linalg.norm(c2_tilde - self.c2) ** 2
        )
    ...
    def fit(self, x_matr, eval_print=100, flush=False):
        ...
        while True:
            ...
            # for debugging purposes
            if self.debug:
                self.eta_list.append(
                    (self.eta_b1, self.eta_b2, self.eta_c0, self.eta_c1, self.eta_c2)
                )
            ...
            # Otherwise it goes to the data fitting step
                ...
                print(
                    "\nold function: {}, new_function: {}".format(
                        str(eval_before), eval_after_p
                    )
                )
            ...
            if eval_print > 0 and np.mod(self.num_iterations, eval_print) == 0:
                print(
                    f"\rFinished iteration {self.num_iterations} of maximal {self.max_iter} function value decreased by: {eval_before-eval_after_p} taking: {time.time()-start} seconds",
                    end="",
                    flush=flush,
                )
