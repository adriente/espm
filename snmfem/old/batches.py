...
class Batches:
    ...
    def __init__(self, X, method="checker", n=4):
        ...
        # Data batches
        self.batches = None
        # For each batch the adj correspond to the rest of the data on which to apply
    ...
    def checker_batches(self):
        """
        Creates the two possibles batches (even and odd) for the checkerboard pattern
        ...
        # batches, adj
        return [self.x_matr[:, checker_vec_odd], self.x_matr[:, checker_vec_even]], [
            adj_odd,
            adj_even,
        ]
    ...
    def checker_adjacency_matrix(self, n):
        ...
        blocks.setdiag(
            [0.5] + [1.0 / 3.0] * (n - 2) + [0.5] + outer_diag * (n - 2), k=n
        )
        blocks.setdiag(
            outer_diag * (n - 2) + [0.5] + [1.0 / 3.0] * (n - 2) + [0.5], k=-n
        )
    ...
    def apply_model(self, model, batch_ind):
        ...
        if self.method == "checker":
            ...
            # Linear regression
            return (
                np.linalg.inv(model.d_matr.T @ model.d_matr)
                @ model.d_matr.T
                @ self.adj[batch_ind]
            )
